...
export async function getSortedPostsData(page = 1, selectedCategory = null) {
  ...
  // Assign series part numbers before filtering (need all posts in series visible)
  const seriesGroups = {};
  allPostsData.forEach(post => {
    if (post.series) {
      if (!seriesGroups[post.series]) seriesGroups[post.series] = [];
      seriesGroups[post.series].push(post);
    }
  });
  Object.values(seriesGroups).forEach(posts => {
    posts.sort((a, b) => new Date(a.date) - new Date(b.date));
    posts.forEach((post, i) => {
      post.seriesPart = i + 1;
      post.seriesTotal = posts.length;
    });
  });
  ...
  export async function getPostData(id) {
    ...
    // Compute series part number by reading all posts
  let seriesPart = null;
  if (matterResult.data.series) {
    const fileNames = fs.readdirSync(postsDirectory);
    const seriesPosts = fileNames
      .map((filename) => {
        const fId = filename.replace(/\.md$/, '');
        const fContents = fs.readFileSync(path.join(postsDirectory, filename), 'utf8');
        const fData = matter(fContents).data;
        return { id: fId, series: fData.series, date: fData.date };
      })
      .filter(p => p.series === matterResult.data.series)
      .sort((a, b) => new Date(a.date) - new Date(b.date));
    seriesPart = seriesPosts.findIndex(p => p.id === id) + 1;
  }
